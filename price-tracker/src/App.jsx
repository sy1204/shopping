...
import { ShoppingBag, X, StickyNote, Plus, LogIn, LogOut, ExternalLink, ArrowUp, ArrowDown, Bell } from 'lucide-react';
...
function App() {
  ...
  const [userAccount, setUserAccount] = useState({ name: "홍길동", email: "hong@example.com", avatar: "H" });
  ...
  const addNewProductByUrl = async () => {
    ...
    try {
      ...
      setNotification({ message: '상품 등록에 실패했습니다. 올바른 상품 URL인지 확인해주세요.', type: 'error' });
    }
  };
  ...
  const getPrimaryMallData = (product) => {
    ...
    // For simplicity, taking the mall with lowest price as primary display
    ...
    <div className="container" style={{
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ fontSize: '14px', color: '#666', marginRight: '10px' }}>
            {isLoggedIn ? `${userAccount.name}님` : '로그인 필요'}
          </div>
          ...
          <div style={{ background: 'white', padding: '1rem', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', border: '1px solid #f0f0f0' }}>
            ...
            <div style={{ position: 'relative', display: 'flex', gap: '8px' }}>
              ...
              <button
                ...
                style={{ padding: '0 12px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
              >
                <Plus size={20} />
              </button>
            </div>
        ...
        <div style={{
          ...
          {!selectedProduct ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
                ...
                <div style={{ padding: '1.5rem', border: '1px solid #f0f0f0', borderRadius: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '1rem' }}>
                    <button onClick={() => adjustTargetPrice(-100)} style={{ width: '32px', height: '32px', borderRadius: '50%', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>-</button>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                    <button onClick={() => adjustTargetPrice(-1000)} style={{ padding: '4px 12px', borderRadius: '20px', border: '1px solid #eee', background: 'white', fontSize: '12px', color: '#666', cursor: 'pointer' }}>-1,000</button>
                    <button onClick={() => adjustTargetPrice(1000)} style={{ padding: '4px 12px', borderRadius: '20px', border: '1px solid #eee', background: 'white', fontSize: '12px', color: '#666', cursor: 'pointer' }}>+1,000</button>
                  </div>
                </div>
                ...
                <div style={{ padding: '1.5rem', border: '1px solid #f0f0f0', borderRadius: '16px', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    ...
                    {isEditingMemo ? (
                      ...
                      <button onClick={() => { setTempMemo(selectedProduct.memo || ""); setIsEditingMemo(true); }} style={{ fontSize: '12px', color: '#888', background: 'none', border: 'none', cursor: 'pointer' }}>수정</button>
                    )}
                  </div>
                  ...
                  {isEditingMemo ? (
                    <textarea
                      ...
                      placeholder="구매 체크포인트, 선물 대상 등 메모..."
                    />
                  ) : (
              ...
              {/* 4. Product Specs (Placeholder) */}
              <div style={{ paddingTop: '1rem', borderTop: '1px solid #f0f0f0' }}>
                <div style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '1rem' }}>상품 정보</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
                  {selectedProduct.specs && selectedProduct.specs.length > 0 ? (
                    selectedProduct.specs.map((spec, i) => (
                      <div key={i} style={{ display: 'flex', fontSize: '13px' }}>
                        <div style={{ width: '80px', color: '#888' }}>{spec.label}</div>
                        <div style={{ color: '#333' }}>{spec.value}</div>
                      </div>
                    ))
                  ) : (
                    <div style={{ color: '#aaa', fontSize: '13px', fontStyle: 'italic' }}>상세 스펙 정보가 수집되지 않았습니다.</div>
                  )}
                </div>
              </div>
            </>
